...
BEISPIEL_URL = "https://github.com/DEIN-NUTZERNAME/DEIN-REPO/raw/main/beispiel.xlsx"

st.set_page_config(layout="wide")
st.title("Interessenpunkte Visualisierung")

st.markdown(
    f"[📥 Beispiel-Datei herunterladen]({BEISPIEL_URL})"
)

uploaded_file = st.file_uploader("Excel-Datei hochladen (wie Vorlage)", type=["xlsx"])

# Daten einlesen (hochgeladen oder Beispiel)
if uploaded_file:
    excel_df = pd.read_excel(uploaded_file)
else:
    st.info("Keine Datei hochgeladen. Beispiel-Datei wird verwendet.")
    excel_df = pd.read_excel(BEISPIEL_URL)
